...
describe('Table.vue', () => {
  ...
  test('renders column', () => {
    ...
    expect(columns.length).toBe(columnData.length);
  });
  ...
  test('renders cell', () => {
    ...
    expect(firstRow.length).toBe(columnData.length);
    expect(lastRow.length).toBe(columnData.length);
  });
  ...
  test('low to high sorting', async () => {
    ...
    const firstCell = firstRow.findAll('td')[columnData.length - 1];
  ...
  test('high to low sorting', async () => {
    ...
    await lastColumn.trigger('click')
    const firstRow = wrapper.findAll('.table-row')[0];
    const lastCell = firstRow.findAll('td')[columnData.length - 1];
    expect(wrapper.vm.sorting.key).toBe("price");
